...
const Devis = () => {
  ...
  const pdfDownload = e => {
    ...
    var element = document.getElementById('devis_pdf')
  ...
  return (
    <div className='big__container'>
      ...
      <div className='pdf__papper' id="devis_pdf">
        <div>  
          <header>
          ...
          <div className='general__infos__container'>
            <div className='left__info'>
              ...
              <div className='left__info__footer'>
                <h2 style={{textDecoration: 'underline'}} >DEVIS N° {last_info.devis}{last_info.date.slice(0, 4)}{last_info.date.slice(5, 7)}{last_info.date.slice(8, 10)}</h2>
              </div>
            </div>
          ...
          <div className='additionel__info'>
              <p>{`Devis arrêté à la somme de ${total + total / 5}dh TTC toutes taxes comprises.`}</p>
              <p>{last_info.secondText} </p>
          </div>
        </div>
      ...
      <div className='download__btn__container'>
        <button className='download__btn' onClick={pdfDownload}>Télécharger le devis PDF</button>
      </div>
    </div>
  );
}
